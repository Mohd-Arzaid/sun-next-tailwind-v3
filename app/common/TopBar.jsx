...
const TopBar = () => {
  ...
  useEffect(() => {
    setUnderlineEnabled(readStoredBoolean(ACCESSIBILITY_STORAGE_KEY));
    setHighContrastEnabled(readStoredBoolean(HIGH_CONTRAST_STORAGE_KEY));
  }, []);

  useEffect(() => {
    if (skipPersist.current) return;

    document.body.classList.toggle("accessibility-underline", underlineEnabled);
    localStorage.setItem(
      ACCESSIBILITY_STORAGE_KEY,
      JSON.stringify(underlineEnabled)
    );
  }, [underlineEnabled]);
  ...
  useEffect(() => {
    ...
    localStorage.setItem(
      ...
      JSON.stringify(highContrastEnabled)
    );
  }, [highContrastEnabled]);
